...
const totalSum = (input) => {
  return input.reduce((acc,curr)=>{
    curr = curr.reduce((acc,curr)=> acc+curr);
    return acc + curr;
  },0);
};
/* ------------------------------------------------------------------------------------------------
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(individualArr=>{
    let newArr = individualArr.filter(arrElement =>{
    ...
    return newArr.map(arrElement => Math.pow(2, arrElement));
  });
};
...
let findMaleAndFemale = (data) => {
  let newArray = data.filter(obj => obj.gender === 'female' || obj.gender === 'male');
  let firstStr ='';
  let resultStr = '';
  firstStr = newArray.map((element) =>{
    return element.name;
  });
