...
const countNumberOfChildren = (arr) => {
  ...
  arr.reduce((acc,curr) => {
    if(curr.children){
      numChildren += curr.children.length;
    }
  },0);
  return numChildren;
};
...
const calculateAverage = (arr) => {
  let total = 0;
  let numOfIndex = 0;
  total = arr.reduce((acc, curr, idx)=>{
    numOfIndex = idx + 1;
    return acc + curr;
  },0);
  return total/numOfIndex;
};
...
const countPrimeNumbers = (arr) => {
  let cnt = 0;
  return arr.reduce((acc, curr)=> (isPrime(curr) ? cnt++ : cnt) ,0);
};
...
const extractStat = (statName, arr) => {
  return arr.reduce((acc,curr) => (curr.stat.name === statName) ? curr : acc ,null);
};
...
const extractChildren = (arr) => {
  let filteredArray = arr.filter(char => char.name.includes('a'));
  let result = [];
