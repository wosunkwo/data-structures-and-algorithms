...
const filterStringsWithVowels = (arr) => {
  let temp = arr.filter(text=>text.match(/[a,e,i,o,u]/));
  return temp;
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let result = arr.filter(stat=> (stat.baseStat > minBaseStat));
  return result;
};
...
const getStatName = (arr, minBaseStat) => {
  let result = arr.filter(stat =>(stat.baseStat > minBaseStat));
  result = result.map(stats => stats = stats.stat.name);
  return result;
};
...
const getCharactersWithoutChildren = (arr) => {
  let result = arr.filter(character=> !(character.children));
  return result;
};
